...
CREATE TABLE IF NOT EXISTS NGUOIDUNG (
    ...
    TrangThai ENUM('HoatDong', 'BiKhoa', 'VoHieuHoa') DEFAULT 'HoatDong',
...
CREATE TABLE IF NOT EXISTS DEACTIVATE_ACCOUNT_OTP (
    Email VARCHAR(255) PRIMARY KEY,
    OTP VARCHAR(10) NOT NULL,
    ExpiresAt DATETIME NOT NULL
);

CREATE TABLE IF NOT EXISTS DONATE_OTP (
    Email VARCHAR(255) PRIMARY KEY,
    OTP VARCHAR(10) NOT NULL,
    ExpiresAt DATETIME NOT NULL
);
...
CREATE TABLE IF NOT EXISTS LICH_SU_XU (
    MaGD INT AUTO_INCREMENT PRIMARY KEY,
    MaND INT NOT NULL,
    LoaiGiaoDich ENUM('NapXu', 'MuaTaiLieu', 'BanTaiLieu', 'TruXuAdmin', 'ThuongXu', 'HoanXu', 'PhatXu', 'TangXu', 'NhanXu') NOT NULL,
...
CREATE TABLE IF NOT EXISTS BOSUUTAP (
    MaBST INT AUTO_INCREMENT PRIMARY KEY,
    MaND INT NOT NULL,
    TenBST VARCHAR(255) NOT NULL,
    MoTa TEXT,
    NgayTao DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (MaND) REFERENCES NGUOIDUNG(MaND) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS BOSUUTAP_TAILIEU (
    MaBST INT NOT NULL,
    MaTL INT NOT NULL,
    NgayThem DATETIME DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (MaBST, MaTL),
    FOREIGN KEY (MaBST) REFERENCES BOSUUTAP(MaBST) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS NGANHANG_CAUHOI (
    MaCauHoi INT AUTO_INCREMENT PRIMARY KEY,
    MaMonHoc INT NOT NULL,
    MaND_Tao INT NOT NULL,
    NoiDung TEXT NOT NULL,
    LoaiCauHoi ENUM('TracNghiem', 'DungSai') DEFAULT 'TracNghiem',
    ...
    FOREIGN KEY (MaND_Tao) REFERENCES NGUOIDUNG(MaND) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS DAP_AN (
    ...
    FOREIGN KEY (MaCauHoi) REFERENCES NGANHANG_CAUHOI(MaCauHoi) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS QUIZ (
    MaQuiz INT AUTO_INCREMENT PRIMARY KEY,
    MaMonHoc INT NOT NULL,
    MaND_Tao INT NOT NULL,
    TieuDe VARCHAR(255) NOT NULL,
    MoTa TEXT,
    ThoiGianLamBai INT NOT NULL DEFAULT 0,
    DaoCauHoi BOOLEAN DEFAULT FALSE,
    TrangThai ENUM('BanNhap', 'CongKhai') DEFAULT 'BanNhap',
    IsDeleted BOOLEAN DEFAULT FALSE,
    NgayTao DATETIME DEFAULT CURRENT_TIMESTAMP,
    NgayCapNhat DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (MaMonHoc) REFERENCES MONHOC(MaMonHoc) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS QUIZ_CAUHOI (
    MaQuiz INT NOT NULL,
    MaCauHoi INT NOT NULL,
    ThuTu INT DEFAULT 0,
    PRIMARY KEY (MaQuiz, MaCauHoi),
    FOREIGN KEY (MaQuiz) REFERENCES QUIZ(MaQuiz) ON DELETE CASCADE,
    FOREIGN KEY (MaCauHoi) REFERENCES NGANHANG_CAUHOI(MaCauHoi) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS KETQUA_QUIZ (
    MaKetQua INT AUTO_INCREMENT PRIMARY KEY,
    MaQuiz INT NOT NULL,
    MaND INT NOT NULL,
    DiemSo DECIMAL(5,2) DEFAULT 0,
    ThoiGianBatDau DATETIME DEFAULT CURRENT_TIMESTAMP,
    ThoiGianNopBai DATETIME NULL,
    TrangThai ENUM('DangLam', 'DaNop') DEFAULT 'DangLam',
    FOREIGN KEY (MaQuiz) REFERENCES QUIZ(MaQuiz) ON DELETE CASCADE,
    FOREIGN KEY (MaND) REFERENCES NGUOIDUNG(MaND) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS CHITIET_KETQUA_QUIZ (
    MaKetQua INT NOT NULL,
    MaCauHoi INT NOT NULL,
    MaDapAnDaChon INT NULL,
    PRIMARY KEY (MaKetQua, MaCauHoi),
    FOREIGN KEY (MaKetQua) REFERENCES KETQUA_QUIZ(MaKetQua) ON DELETE CASCADE,
    FOREIGN KEY (MaCauHoi) REFERENCES NGANHANG_CAUHOI(MaCauHoi) ON DELETE CASCADE,
    FOREIGN KEY (MaDapAnDaChon) REFERENCES DAP_AN(MaDapAn) ON DELETE SET NULL
);
