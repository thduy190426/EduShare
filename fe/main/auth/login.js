...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (lastLoginMethod && loginForm) {
    ...
    loginForm.insertAdjacentHTML(
      "beforebegin",
      `<div style="text-align:center; animation: fadeInDown 0.5s ease-out;">${badgeHtml}</div>`,
    );
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        submitBtn.innerHTML =
          '<i class="fa-solid fa-spinner fa-spin" style="margin-right: 8px;"></i>&nbsp; Đang đăng nhập...';
        ...
        if (!response.ok) {
          ...
          if (totpCode) {
            ...
            if (!verifyRes.ok) {
              Swal.fire(
                "Lỗi",
                verifyData.message || "Mã xác thực không hợp lệ",
                "error",
              );
              return;
            }
          ...
          try {
            ...
            console.error("Lỗi parse token khi đăng nhập:", e);
          }
